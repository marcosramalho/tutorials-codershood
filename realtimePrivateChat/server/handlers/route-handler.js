...
class RouteHandler{
	...
	async loginRouteHandler(request, response){
		...
		if(data.username === '' || data.username === null) {
			...
			response.status(ONSTANTS.SERVER_ERROR_HTTP_CODE).json({
				error : true,
				message : CONSTANTS.PASSWORD_NOT_FOUND
			});
		} else {
			try {
				const [result] = await queryHandler.getUserByUsername(data);
				...
				if(result ===  null || result === undefined) {
					...
					await queryHandler.makeUserOnline(result.id);
	...
	async userSessionCheckRouteHandler(request, response){
		...
		if (userId === '' && userId === undefined) {
			...
			try {
				const username = await queryHandler.userSessionCheck(userId);
